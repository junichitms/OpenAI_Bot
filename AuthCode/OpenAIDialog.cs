...
namespace OpenAI_Bot.Dialogs
{
    public class OpenAIDialog : ComponentDialog
    {
        ...
        private int auth = 0; //認証の検証コードの入力かどうか判定するために利用
            ...
            : base(nameof(OpenAIDialog))
        {
            ...
            var waterfallSteps = new WaterfallStep[]
           {
            ...
            AddDialog(new OAuthPrompt(nameof(OAuthPrompt), new OAuthPromptSettings
            {
                 ...
                 Text = "ログインして下さい。検証コードが別ウィンドウで表示されますので、そのコードを入力して下さい。",
                 Title = "Login",
        ...
        private async Task<DialogTurnResult> LoginStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            {
                if (auth == 0 ) //loginが発生した際の処理
                {
                    await stepContext.Context.SendActivityAsync(MessageFactory.Text("ログイン済みです"), cancellationToken);

                    string _token = tokenResponse.Token;
                    ...
                    await stepContext.Context.SendActivityAsync(MessageFactory.Text(me.DisplayName+"さん。問い合わせ内容を入力して下さい。"), cancellationToken);
                }
